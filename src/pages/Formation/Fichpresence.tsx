...
const FichePresence = () => {
    ...
    Promise.all([
      ...
      const formatted: Participant[] = participants.map((p: any) => {
        ...
        presenceDates.forEach(date => {
          presence[date] = presenceMap[`${p.employe.id}-${date}`] ?? true; // par défaut présent
        });
        return {
          id: p.employe.id,
  ...
  const handlePresenceChange = (participantId: number, date: string, isPresent: boolean) => {
    setParticipantData(prev =>
      prev.map(part =>
        part.id === participantId
          ? { ...part, presence: { ...part.presence, [date]: isPresent } }
          : part
      )
    );
  };
    ...
    try {
      for (const participant of participantData) {
        for (const date of presenceDates) {
          const statut = participant.presence[date] ? "PRESENT" : "ABSENT";
  ...
  return (
    <div className="bg-gray-50 dark:bg-gray-900 p-4 min-h-screen">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 p-6 mb-4">
        {formation ? (
          <>
            <h2 className="text-lg font-bold text-gray-900 dark:text-white mb-4">Fiche de présence</h2>
          ...
          <TableHeader>
            <TableRow className="bg-gray-100 dark:bg-gray-700">
              ...
              {presenceDates.map(date => (
                <TableCell isHeader key={date} className="px-4 py-2 text-gray-600 dark:text-gray-300">{date}</TableCell>
              ))}
            </TableRow>
          </TableHeader>
